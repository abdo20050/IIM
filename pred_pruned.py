...
def apply_pruning(model, amount):
    ...
    This deactivates the least weighted nodes (weights) across the entire model.
    """
    parameters_to_prune = []
    ...
    # Remove pruning hooks to make the changes permanent and compatible with torch.compile
...
def main():
    ...
    parser.add_argument('--prune', type=float, default=0.0, help='Pruning percentage (0.0 to 1.0) to deactivate least weighted nodes')
    ...
    if args.prune > 0:
        print(f"[*] Applying {args.prune*100:.1f}% global unstructured pruning to model weights...")
        apply_pruning(net, args.prune)
    ...
    while True:
        ...
        if args.gt is not None:
            gt_vis = gt_map.get(frame_idx, gt_map.get(str(frame_idx), gt_map.get(os.path.splitext(os.path.basename(args.input))[0], None)))
            if gt_vis is not None and 'points' in gt_vis and getattr(gt_vis['points'], 'size', 0) > 0:
                ...
                if pts.ndim == 1:
                    pts = pts.reshape(1, 2)
                if args.verbose:
                    print(f"GT points (frame {frame_idx}): {len(pts)}")
                for gx, gy in pts:
                    try:
        ...
        if args.gt is not None:
            # try integer key (frame index) first, then string, then basename
            key = frame_idx
            ...
            if gt is not None:
                ...
                # detection metrics via Hungarian matching
                # Prefer tracked (Kalman) points for evaluation to reduce FPs from duplicates/noise.
                try:
                    if 'tracked' in locals() and tracked is not None and tracked.size > 0:
                        preds = np.array([row[:2] for row in tracked])
                    else:
                        preds = np.array(pred_data['points']) if pred_data['num'] > 0 else np.empty((0, 2))
                except Exception:
                    preds = np.array(pred_data['points']) if pred_data['num'] > 0 else np.empty((0, 2))
                ...
                # print per-frame stats (frame numbers are zero-based to match GT CSV)
        ...
        # verbose per-frame timing (zero-based index)
        if args.verbose:
